...
$stmt->bind_param("ii", $userId, $user_id);
$stmt->execute();
$userPosts = $stmt->get_result();
$stmt->close();

$stmt = $conn->prepare("SELECT replies.*, posts.title AS post_title FROM replies
INNER JOIN posts ON replies.post = posts.id
WHERE replies.replier = ?
ORDER BY replies.timestamp DESC");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$userReplies = $stmt->get_result();
$stmt->close();
